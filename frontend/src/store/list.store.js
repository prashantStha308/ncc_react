...
export const useListStore = create((set) => ({
    ...
    setListDetails: (details) => set({ listDetails: details }),

    // Fetch all task lists
    fetchAllLists: async (userId) => {
  ...
  // Fetch a single list by ID
    fetchListById: async (listId) => {
        ...
        try {
            ...
            set((state) => ({
            listDetails: { ...state.listDetails, [listId]: res.data },
            loading: false,
            }));
        } catch (err) {
    ...
    createList: async (ownerId, objectData) => {
        set({ loading: true, error: null });
        try {
            const res = await createTaskList(ownerId, objectData);
            set((state) => ({
